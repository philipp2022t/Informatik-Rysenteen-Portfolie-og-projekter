...
function preload() {
  keyImg = loadImage('billeder/key.png');
...
function encrypt() {
  ...
  for (let i = 0; i < w; i++) {
    for (let j = 0; j < h; j++) {
      ...
      let krypteretColor = color((getPixelValueImg(0,i,j)+getPixelValueKey(0,i,j))%256,
      (getPixelValueImg(1,i,j)+getPixelValueKey(1,i,j))%256,
      (getPixelValueImg(2,i,j)+getPixelValueKey(2,i,j))%256);
